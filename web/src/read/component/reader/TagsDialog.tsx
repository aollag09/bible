    ...
    props.tags.forEach(tag => {
        ...
        switch (+tag.getTagCase()) {
            ...
            case Tag.TagCase.WHATTAG:
                tagSpecifics.push(<DialogField key={tag.getId() + "what"} field="what" value={tag.getWhattag()?.getWhat()} />)
                tagSpecifics.push(<DialogField key={tag.getId() + "details"} field="details" value={tag.getWhattag()?.getDetails()} />)
                break;
            case Tag.TagCase.WHOTAG:
                tagSpecifics.push(<DialogField key={tag.getId() + "who"} field="who" value={tag.getWhotag()?.getWho().toString()!} />)
                break;
            case Tag.TagCase.WHERETAG:
                tagSpecifics.push(<DialogField key={tag.getId() + "where"} field="where" value={tag.getWheretag()?.getWhere()!} />)
                tagSpecifics.push(<DialogField key={tag.getId() + "latitude"} field="latitude" value={tag.getWheretag()?.getLatitude().toString()!} />)
                tagSpecifics.push(<DialogField key={tag.getId() + "longitude"} field="longitude" value={tag.getWheretag()?.getLongitude().toString()} />)
                break;
            case Tag.TagCase.WHENTAG:
                tagSpecifics.push(<DialogField key={tag.getId() + "year"} field="year" value={tag.getWhentag()?.getYear().toString()} />)
                break;
            case Tag.TagCase.HOWTAG:
                tagSpecifics.push(<DialogField key={tag.getId() + "how"} field="how" value={tag.getHowtag()?.getHow()} />)
                tagSpecifics.push(<DialogField key={tag.getId() + "howdetails"} field="details" value={tag.getHowtag()?.getDetails()} />)
                break;
        }
